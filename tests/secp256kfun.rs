...
use dleq::engines::{secp256kfun::Secp256k1Engine, DLEqEngine};
...
fn secp256k1_scalar_bits() {
    ...
    assert_eq!(
        hex::encode(
            &Secp256k1Engine::little_endian_bytes_to_private_key(key)
                .unwrap()
                .to_bytes()
        ),
        hex::encode(&key_rev)
...
fn alt_secp256k1() {
    ...
    alt.extend(
        &Sha256::new()
            .chain(secp256kfun::G.to_bytes_uncompressed())
            .finalize(),
    );
    assert_eq!(
