...
fn test_with<EngineA: DLEqEngine, EngineB: DLEqEngine>() {
    ...
    assert_eq!(
        ...
        hex::encode(EngineA::public_key_to_bytes(&EngineA::to_public_key(
            &skey_a
        )))
    );
    assert_eq!(
        hex::encode(EngineB::public_key_to_bytes(&pkey_b)),
        hex::encode(EngineB::public_key_to_bytes(&EngineB::to_public_key(
            &skey_b
        )))
    );
...
fn k256_secp256kfun_interchangability() {
    ...
    let (proof, skey_a, skey_b) =
        DLEqProof::<k256::Secp256k1Engine, secp256kfun::Secp256k1Engine>::new(&mut thread_rng());
    ...
    let proof = DLEqProof::<secp256kfun::Secp256k1Engine, k256::Secp256k1Engine>::deserialize(
    ...
    assert_eq!(
        ...
        hex::encode(k256::Secp256k1Engine::public_key_to_bytes(
            &k256::Secp256k1Engine::to_public_key(&skey_a)
        ))
    );
    assert_eq!(
        hex::encode(k256::Secp256k1Engine::public_key_to_bytes(&pkey_b)),
        hex::encode(secp256kfun::Secp256k1Engine::public_key_to_bytes(
            &secp256kfun::Secp256k1Engine::to_public_key(&skey_b)
        ))
    );
}
