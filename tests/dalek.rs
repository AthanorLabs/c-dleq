...
use dleq::engines::{ed25519::Ed25519Engine, ristretto::RistrettoEngine, DLEqEngine};
...
fn ed25519_scalar_bits() {
    ...
    assert_eq!(
        hex::encode(
            Ed25519Engine::little_endian_bytes_to_private_key(key)
                .unwrap()
                .as_bytes()
        ),
        hex::encode(&key)
...
fn ristretto_scalar_bits() {
    ...
    assert_eq!(
        hex::encode(
            RistrettoEngine::little_endian_bytes_to_private_key(key)
                .unwrap()
                .as_bytes()
        ),
        hex::encode(&key)
...
fn alt_ed25519() {
    ...
    assert_eq!(
        CompressedEdwardsY::from_slice(&hash_G)
            .decompress()
            .unwrap()
            .mul_by_cofactor(),
...
fn alt_ristretto() {
    assert_eq!(
        RistrettoPoint::from_hash(
            Blake2b::new().chain(RISTRETTO_BASEPOINT_POINT.compress().as_bytes())
        ),
