...
impl DLEqEngine for RistrettoEngine {
    ...
    fn bytes_to_public_key(key: &[u8]) -> DLEqResult<Self::PublicKey> {
        CompressedRistretto::from_slice(key)
            .decompress()
    ...
    fn generate_commitments<R: RngCore + CryptoRng>(
        ...
        for i in 0..bits {
            ...
            let (commitment, commitment_minus_one) = if (key[i / 8] >> (i % 8)) & 1 == 1 {
                (
                    &commitment_base + &RISTRETTO_BASEPOINT_POINT,
                    commitment_base,
                )
            } else {
                (
        ...
        let pubkey = &Scalar::from_canonical_bytes(key)
            .expect("Generating commitments for an invalid Ristretto key")
        ...
        debug!(
            "Generated DL Eq proof for Ristretto pubkey {}",
        ...
        let s = k
            - (*key
                * Scalar::from_bytes_mod_order(
                    Blake2b::digest(&to_hash)[..32].try_into().unwrap(),
                ));
    ...
    fn verify_signature(
        ...
        if RistrettoPoint::vartime_double_scalar_mul_basepoint(&c, &public_key, &signature.s)
            == signature.R
        {
            Ok(())
        } else {
