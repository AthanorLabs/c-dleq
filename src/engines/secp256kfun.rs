...
impl DLEqEngine for Secp256k1Engine {
    ...
    fn bytes_to_public_key(key: &[u8]) -> DLEqResult<Self::PublicKey> {
        Point::from_bytes(key.try_into().map_err(|_| DLEqError::InvalidPoint)?)
    ...
    fn generate_commitments<R: RngCore + CryptoRng>(
        ...
        for i in 0..bits {
            let blinding_key = if i == (bits - 1) {
                ...
                s!(-blinding_key_total * inv_power_of_two)
                    .mark::<NonZero>()
            ...
            power_of_two = s!(power_of_two * two)
                .mark::<NonZero>()
                .expect("Power of two is zero");
            ...
            let normalize_point = |point: Point<Jacobian, Public, Zero>| {
                point
                    .mark::<Normal>()
                    .mark::<NonZero>()
                    .expect("Generated zero commitment")
            };
            let (commitment, commitment_minus_one) = if (key[i / 8] >> (i % 8)) & 1 == 1 {
                (
                    normalize_point(g!(commitment_base + G)),
                    commitment_base.mark::<Normal>(),
                )
            } else {
        ...
        let decoded_key = Self::little_endian_bytes_to_private_key(key)
            .expect("Generating commitments for an invalid secp256k1 key");
        ...
        debug_assert_eq!(
            ...
            &Self::reconstruct_key(commitments.iter().map(|c| &c.commitment))
                .expect("Reconstructed our own key to 0"),
            &pubkey
        );
        debug!(
            "Generated DL Eq proof for secp256k1 pubkey {}",
    ...
    fn compute_signature_s(
        ...
        s!(nonce + (challenge * key))
            .mark::<NonZero>()
            .expect("Generated zero s value")
    }
    ...
    fn compute_signature_R(
        ...
        let challenge = Scalar::from_bytes_mod_order(challenge);
        Ok(g!(s_value * ALT_BASEPOINT - challenge * key)
            .mark::<Normal>()
            .mark::<NonZero>()
    ...
    fn reconstruct_key<'a>(
        ...
        for comm in commitments {
            ...
            power_of_two = s!(power_of_two * two)
                .mark::<NonZero>()
                .expect("Generated zero power of two");
        }
        res.mark::<Normal>()
            .mark::<NonZero>()
            .ok_or(DLEqError::InvalidPoint)
        ...
        if sig.len() != Self::signature_len() {
            ...
            Ok(Self::Signature {
                R: Point::from_bytes(
                    sig[..33]
                        .try_into()
                        .expect("Signature was correct length yet didn't have a 33-byte point"),
                )
