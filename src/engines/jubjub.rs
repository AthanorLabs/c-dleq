...
use crate::{
    engines::{
        ff_group::{FfGroupConversions, FfGroupEngine},
        BasepointProvider,
    },
...
impl FfGroupConversions for JubjubConversions {
    ...
    fn bytes_to_point(bytes: &[u8]) -> DLEqResult<Self::Point> {
        let point =
            SubgroupPoint::from_bytes(bytes.try_into().map_err(|_| DLEqError::InvalidPoint)?);
...
impl BasepointProvider for JubjubBasepoints {
    ...
    fn alt_basepoint() -> Self::Point {
        SubgroupPoint::from_bytes(&hex!(
            "1e29013fa7b422934f20624a8cf027465acc5aaa15c1f73de61538b67aa43151"
        ))
        .unwrap()
    }
}
